...
let playerSign = "X"; //inti State player is X
let runBot = true;
...
function getIdVal(classname){
    return document.querySelector(".box" + classname).id; //return value of id
}

function checkIdSign(val1, val2, val3, sign){
    if(getIdVal(val1) == sign && 
    getIdVal(val2) == sign && 
    getIdVal(val3) == sign) {
        return true;
    }
...
function selectWinner() {
    ...
    || checkIdSign(3,5,7, playerSign)) {
        ...
        bot(runBot);
        setTimeout(() => {
            //delay to show result box
            resultBox.classList.add("show");
            playBoard.classList.remove("show");
        }, 700);
        wonText.innerHTML = `Player ${playerSign}</p> won the game!`;
    }else{ 
        if(getIdVal(1) != "" && 
        getIdVal(2) != "" && 
        getIdVal(3) != "" && 
        ...
        getIdVal(9) != ""){
            runBot = false; 
            bot(runBot);
            setTimeout(()=>{ //when match drawn, hide playboard show the result box after 700ms
                resultBox.classList.add("show");
                playBoard.classList.remove("show");
            }, 700); //1s = 1000ms
